...
<div id="k2Container" class="creativeshowim counterplan itemView<?php echo $k2ContainerClasses; ?>"> <?php echo $this->item->event->BeforeDisplay; ?> <?php echo $this->item->event->K2BeforeDisplay; ?>
    <article>
			...
			<div class="itemVideoBlock" id="itemVideoAnchor">
						...
						<div class="video-container itemVideoEmbedded">
 <!-- just for cpiano -->
<?php 
  $isYoutube=false;
  $isEmbedCode=false;
  $videocode="";
   $videourl = $this->item->video;
   if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $videourl, $match)) {
    $video_id = $match[1];
    $videourl="https://www.youtube.com/embed/".$video_id."?autoplay=1";
    $isYoutube=true;
   }
   if (stristr($videourl,"<embed") || stristr($videourl,"<object") || stristr($videourl,"<iframe") || stristr($videourl,"<video")){
       $isEmbedCode=true;
       $videocode=$videourl;
   }
  // $videourl = str_replace("/watch?v=", "/embed/", $this->item->video);
    //  $videourl .= "?autoplay=1";
  
   if ($isYoutube && !$isEmbedCode){
?>
  ...
  <!-- end just for cpiano -->		
   ...
   <?php else: ?>
			...
			<?php echo $this->item->event->AfterDisplayTitle; ?> <?php echo $this->item->event->K2AfterDisplayTitle; ?>
                            ...
                            $file=  str_replace("cache","src",JPATH_BASE.$this->item->image);
                            $file=  str_replace("_L.jpg",".jpg",$file);
                              $file=  str_replace("_XL.jpg",".jpg",$file);
                                 $file=  str_replace("_M.jpg",".jpg",$file);
                                    $file=  str_replace("_S.jpg",".jpg",$file);
                                                $file=  str_replace("_XS.jpg",".jpg",$file);
                            $imgsize=getimagesize($file); //original file
                          //  die(print_r($imgsize,true));
                            $imgw=$imgsize[0];
                            $imgh=$imgsize[1];
                         
                        //  die ($file."--".print_r($imgw,true));
                        if ($imgw > 400){
                       ...
                       window.setTimeout(function(){
                            imagesLoaded( '.itemImage', function() {
                                var img = document.getElementById('itemImageImgTag'); 
                                ...
                                if ( (width/height) < 
                                    ...
                                    //if img is too tall than  compress the view
                                    jQuery('#k2Container').removeClass('creativeshowim');
                                }
                            });
                        },400);  
                        </script>
                       ...
                       title="<?php echo JText::_('K2_CLICK_TO_PREVIEW_IMAGE'); ?>"> 
                            <img id="itemImageImgTag" src="<?php echo $this->item->image; ?>" 
		...
		<?php if(!empty($this->item->fulltext) ||!empty($this->item->introtext) ){ ?>
			<div class="itemBody articleabitoverimg">
						...
						<?php if($params->get('itemFullText')):
                                                 /*   $fulltextsignature=md5($this->item->fulltext);
                                                
                                                    $introtextsignature=md5($this->item->introtext);
                                                    $fullTextDifferentFromIntro=$fulltextsignature!=$introtextsignature;
                                                    print "<!-- ftsign $fulltextsignature fulltxt ".$this->item->fulltext." introsign $introtextsignature  intotxt: ".$this->item->introtext." -->";
                                                 */   ?>
                            
						<div class="itemFullText">
									<?php echo (!empty($this->item->fulltext)) ? $this->item->fulltext : ""; ?>
						</div>
						<?php endif; ?>
                ...
                <?php }else{ //no text 
                           if ($imageIsLowres){ ?>
                            <img id="itemImageImgTag" class="notxtinart" src="<?php echo $this->item->image; ?>" 
                                 alt="<?php if(!empty($this->item->image_caption)) 
                                     echo K2HelperUtilities::cleanHtml($this->item->image_caption); 
                                 else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" 
                                 style="
